...
uint32_t acquisitionTaskBuffer[ 1024 ];
osStaticThreadDef_t acquisitionTaskControlBlock;
const osThreadAttr_t acquisitionTask_attributes = {
  .name = "acquisitionTask",
  .cb_mem = &acquisitionTaskControlBlock,
  .cb_size = sizeof(acquisitionTaskControlBlock),
  .stack_mem = &acquisitionTaskBuffer[0],
  .stack_size = sizeof(acquisitionTaskBuffer),
  .priority = (osPriority_t) osPriorityNormal,
};
...
osStaticMessageQDef_t acquisitionQueueControlBlock;
const osMessageQueueAttr_t acquisitionQueue_attributes = {
  .name = "acquisitionQueue",
  .cb_mem = &acquisitionQueueControlBlock,
  .cb_size = sizeof(acquisitionQueueControlBlock),
  .mq_mem = &acquisitionQueueBuffer,
  .mq_size = sizeof(acquisitionQueueBuffer)
};
/* Definitions for displayQueue */
osMessageQueueId_t displayQueueHandle;
uint8_t displayQueueBuffer[ 1 * sizeof( uint16_t ) ];
...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of acquisitionQueue */
  acquisitionQueueHandle = osMessageQueueNew (1, sizeof(uint16_t), &acquisitionQueue_attributes);
...
static void MX_TIM1_Init(void)
{
  ...
  htim1.Instance = TIM1;
  htim1.Init.Prescaler = 7999;	//modificato da 0
  htim1.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim1.Init.Period = 999;		//modificato da 65535
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /**/
  GPIO_InitStruct.Pin = IO10_GPIO_Pin;
  GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
...
__weak void StartAcquisitionTask(void *argument)
{
